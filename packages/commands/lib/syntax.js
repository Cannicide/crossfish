    ...
    static parseInnerArgument(arg) {
        ...
        if (data) {
            if (data.match(/\|/)) {
                ...
                if (result.choices.some(c => isNaN(c)))
                    result.type = ArgType.String;
                else if (result.choices.every(c => Number.isInteger(Number(c))))
                    result.type = ArgType.Int;
                else
                    result.type = ArgType.Float;
            }
            else if (data.match(/[<>]/)) {
                ...
                if (!isLength) {
                    if (min.length)
                        result.min = Math.max(...min.map(num => Number(num))); // Ex: x > 3 and x > 4 should cause min to be 4 (the max of the two)
                    if (max.length)
                        result.max = Math.min(...max.map(num => Number(num))); // Ex: x < 3 and x < 4 should cause max to be 3 (the min of the two)
                    if (min.concat(max).every(c => !c.match(/\./)))
                        result.type = ArgType.Int;
                    else
                        result.type = ArgType.Float;
                }
                    ...
                    if (min.length)
                        result.minLength = Math.max(...min.map(num => Number(num))); // Ex: l > 3 and l > 4 should cause min to be 4 (the max of the two)
                    if (max.length)
                        result.maxLength = Math.min(...max.map(num => Number(num))); // Ex: l < 3 and l < 4 should cause max to be 3 (the min of the two)
                    result.type = ArgType.String;
                }
            }
            else {
