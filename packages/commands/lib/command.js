...
class CrossfishCommand {
    ...
    addRawSub(name, description = "No description provided.", type) {
        ...
        if (type == SubType.Hybrid) {
            ...
            if (!this.data.subs.has(subgroup))
                this.addRawSub(subgroup, subgroup + ".", SubType.Subgroup);
            const sub = new SlashCommandSubcommandBuilder().setName(subcommand).setDescription(description);
            const group = this.data.subs.get(subgroup);
     ...
     * @private
     */
    rawArgument({ name, description = "No description provided.", type = "string", optional = false, subcommand, subgroup, choices, max, min, maxLength, minLength }) {
        if (isDefined(name))
            ErrorUtil.noexist(name, "Argument name was not provided");
        if (isDefined(name))
            ErrorUtil.badtype(name, "string", "argument name");
        ...
        if (isDefined(subcommand))
            ErrorUtil.badtype(subcommand, "string", "argument subcommand name");
        if (isDefined(subgroup))
            ErrorUtil.badtype(subgroup, "string", "argument subgroup name");
        if (isDefined(subgroup))
            ErrorUtil.noexist(subcommand, "Argument subcommand must be provided if a subgroup is provided");
        if (isDefined(choices))
            ErrorUtil.badtype(choices, "array", "argument choices");
        if (isDefined(max))
            ErrorUtil.badtype(max, "number", "argument max value");
        if (isDefined(min))
            ErrorUtil.badtype(min, "number", "argument min value");
        if (isDefined(maxLength))
            ErrorUtil.badtype(maxLength, "number", "argument maxLength value");
        if (isDefined(minLength))
            ErrorUtil.badtype(minLength, "number", "argument minLength value");
        if (isDefined(max) && isDefined(min))
            ErrorUtil.pred(() => (max ?? 0) < (min ?? 0), "Invalid argument max and min values; max must be larger than min");
        if (isDefined(maxLength) && isDefined(minLength))
            ErrorUtil.pred(() => (maxLength ?? 0) < (minLength ?? 0), "Invalid argument max and min values; maxLength must be larger than minLength");
        if (isDefined(choices))
            ErrorUtil.minmax(choices?.length, "number of choices", 1, 25);
        ...
        if (isDefined(description))
            ErrorUtil.minmax(description.length, "argument description length", 1, 100);
        if (isDefined(name))
            ErrorUtil.minmax(name?.length, "argument name length", 1, 32);
        if (isDefined(name))
            ErrorUtil.slashname(name, "argument");
        // Check datatype
        ...
        // Add Argument
        let addArgument;
        if (ArgTypeUtil.isChannel(parsedType))
            addArgument = builder.addChannelOption.bind(builder);
        else if (parsedType == ArgType.User)
            addArgument = builder.addUserOption.bind(builder);
        else if (parsedType == ArgType.Bool)
            addArgument = builder.addBooleanOption.bind(builder);
        else if (parsedType == ArgType.Role)
            addArgument = builder.addRoleOption.bind(builder);
        else if (parsedType == ArgType.Mention)
            addArgument = builder.addMentionableOption.bind(builder);
        else if (parsedType == ArgType.Float)
            addArgument = builder.addNumberOption.bind(builder);
        else if (parsedType == ArgType.Int)
            addArgument = builder.addIntegerOption.bind(builder);
        else if (parsedType == ArgType.File)
            addArgument = builder.addAttachmentOption.bind(builder);
        else
            addArgument = builder.addStringOption.bind(builder);
        addArgument((arg) => {
            arg.setName(name)
            ...
            if (isDefined(max))
                arg.setMaxValue(max);
            if (isDefined(maxLength))
                arg.setMaxLength(maxLength);
            if (isDefined(minLength))
                arg.setMinLength(minLength);
            if (ArgTypeUtil.isChannel(parsedType))
                arg.addChannelTypes(...parsedType);
            if (choices)
                arg.addChoices(...choices.map(choice => ({ name: choice, value: choice })));
    ...
    require(permOrRole) {
        ...
        if (typeof permOrRole !== "string") {
            const perm = permOrRole;
            if (Object.values(PermissionsBitField.Flags).includes(perm))
                this.data.requires.perms.add(perm);
        }
        else {
            const role = permOrRole;
            const value = role.replace("@", "");
            if (perms.includes(role.toLowerCase()) && !role.startsWith("@"))
                this.data.requires.perms.add(PermissionsBitField.resolve(value));
            else
                this.data.requires.roles.add(value);
        }
        return this;
    }
    /**
    ...
    descriptions(descriptions) {
        ...
        for (const rawKey of Object.keys(descriptions)) {
            ...
            if (typeof descriptions[rawKey] == "string")
                arg.setDescription(descriptions[rawKey]);
            else {
                const localizations = descriptions[rawKey];
                if ("default" in localizations) {
                    arg.setDescription(localizations["default"]);
                    delete localizations["default"];
                }
                else if ("en-US" in localizations)
                    arg.setDescription(localizations["en-US"]);
                else if ("en-GB" in localizations)
                    arg.setDescription(localizations["en-GB"]);
                else
                    ErrorUtil.noexist(null, "Every argument/subcommand must provide either a 'default' description or an 'en-US'/'en-GB' localized description");
    ...
    autocomplete(methods) {
        ...
        for (const rawKey of Object.keys(methods)) {
            ...
            ErrorUtil.pred(() => (this.data.subs.get(key).choices?.length ?? 0) > 0, `The argument '${rawKey}' provided to autocomplete() has choices defined, and cannot be made autocompletable`);
            this.data.subs.get(key).setAutocomplete(true);
    ...
    */
    action(method) {
        ...
        if (typeof method === "function")
            ...
            for (const rawKey of Object.keys(method)) {
                ...
                if (key != def)
                    ErrorUtil.hasnot(key, this.data.subs, `The subcommand or subgroup '${rawKey}' provided to action() is nonexistent`);
            ...
            this.data.action = interaction => {
                ...
                if (methods.has(combo))
                    methods.get(combo)(interaction); // High priority to specific subcommands in subgroups
                else if (methods.has(subgroup))
                    methods.get(subgroup)(interaction); // Lower priority to subgroups in general
                else if (methods.has(def))
                    methods.get(def)(interaction); // Lowest priority to default handler
            };
        }
        return this.build();
    }
    ...
    build() {
        ...
        if (CrossfishCommand.docs && !this.data.docs)
            this.docs(CrossfishCommand.docs);
        if (this.data.docs)
            this.descriptions(this.data.docs);
        // Build command
        this.data.JSON = this.builder.toJSON();
        CrossfishHandler.setCommand(this.data.JSON.name, this);
    }
}
