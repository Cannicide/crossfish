...
class ArgTypeUtil {
    static get(value) {
        ...
        if (value.endsWith("channel")) {
            ...
            return ArgType.User;
        else if (["boolean", "bool"].includes(value))
            return ArgType.Bool;
        else if (value == "role")
            return ArgType.Role;
        else if (["mention", "mentionable"].includes(value))
            return ArgType.Mention;
        else if (["num", "number", "float"].includes(value))
            return ArgType.Float;
        else if (["int", "integer", "intg"].includes(value))
            return ArgType.Int;
        else if (["attachment", "file", "image"].includes(value))
            return ArgType.File;
        else if (["string", "str"].includes(value))
...
class SubTypeUtil {
    static getType(subgroup, subcommand) {
        if (subgroup && subcommand)
            return SubType.Hybrid;
        if (subcommand)
            return SubType.Subcommand;
        if (subgroup)
